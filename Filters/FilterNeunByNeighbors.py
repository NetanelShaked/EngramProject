...
def squere_info(image_path, csv_path):
    ...
    channel = 2
    csv=pd.DataFrame(shuffle(csv)).reset_index().iloc[:min(len(csv),1000)]
    for idx_main,row_main in csv.iterrows():
        x_pixel = int(row_main['x'])
        y_pixel = row_main['y']
        relevant_csv = csv[(csv['y'] > y_pixel - distance) & (y_pixel + distance > csv['y'])]
        relevant_csv = relevant_csv[(relevant_csv['x'] > x_pixel - distance) & (x_pixel + distance > relevant_csv['x'])]
        summarize_list = []
        ...
        for idx, row in relevant_csv.iterrows():
            ...
            cell_locations[y_cell+distance-y_pixel-cell_radius_in_pixel:y_cell+distance-y_pixel+cell_radius_in_pixel,x_cell+distance-x_pixel-cell_radius_in_pixel:x_cell+distance-x_pixel+cell_radius_in_pixel]=1
...
if __name__ == '__main__':
    ...
    csv = r'D:\Lab\Data_from_lab\N2-20210214T082519Z-012\N2\1h\csv files\separate files\-3000.csv'
